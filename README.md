...
```
ブラウザで開いたら、右上の **?（使い方ガイド）** を押すと手順が出る。**右ペインのインスペクターを見ながら**左で操作するのがコツ。

1. タスクを**追加** → `POST /api/todos 201`（+ `HX-Trigger` トースト）
2. **✓ で完了トグル** → `PATCH`（部分更新）
3. 名前を**編集して Enter** → `PUT`（全置換・冪等）
4. **削除** → `DELETE`（`hx-confirm` の確認後に行が消える）
5. **同名を再追加** → `409 Conflict`（エラーも可視化）
6. 検索ボックスに**入力** → 300ms 後に絞込（`hx-trigger` + クエリ文字列）/ All・Active・Done でフィルタ
7. 右上の**テーマ切替** → Cookie 保存 + `HX-Redirect` でリロード
